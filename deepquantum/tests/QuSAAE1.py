...
from deepquantum.gates.qmath import partial_trace, partial_trace_batched, partial_trace_batched2, batched_kron, \
    batched_kron2
...
if __name__ == '__main__':
    ...
    g = nn.functional.normalize(torch.rand(batchsize, 1, 2 ** dimg) + torch.rand(batchsize, 1, 2 ** dimg) * 1j, p=2,
                                dim=2)
    # m = nn.functional.normalize(torch.rand( 1, 2 ** N) + torch.rand( 1, 2 ** N) * 1j, p=2, dim=1)
    # g = nn.functional.normalize(torch.rand( 1, 2 ** dimg) + torch.rand( 1, 2 ** dimg) * 1j, p=2,
    #                             dim=1)
