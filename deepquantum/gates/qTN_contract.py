...
def _SingleGateOperation_TN_contract_Rho(nqubits, wires, matrix,\
                                     MPDO:torch.Tensor, batch_mod:bool=False)->torch.Tensor:
    '''
    2022-02-21 ssy
    '''
    # t1 = time.time()
    # print("using _SingleGateOperation_TN_contract_Rho")
    if nqubits == -1 or wires == -1:
        raise ValueError("_SingleGateOperation_TN_contract_Rho input error! cannot TN_contract")
    if batch_mod==False:
        assert MPDO.ndim == 2*nqubits
        #左乘
        permute_shape = list( range(2*nqubits) )
        permute_shape[wires] = 2*nqubits - 1
        permute_shape[2*nqubits - 1] = wires
        MPDO = MPDO.permute(permute_shape).unsqueeze(-1)
        # t2 = time.time()
        MPDO = ( matrix @ MPDO ).squeeze(-1)
        # t3 = time.time()

        MPDO = MPDO.permute(permute_shape)
        
        
        #右乘
        permute_shape = list( range(2*nqubits) )
        permute_shape[wires+nqubits] = 2*nqubits - 1
        permute_shape[2*nqubits - 1] = wires+nqubits
        MPDO = MPDO.permute(permute_shape).unsqueeze(-2)
        # t4 = time.time()
        MPDO = ( MPDO @ matrix.permute(1,0).conj() ).squeeze(-2)
        # t5 = time.time()
        # print("乘法耗时：",t5 - t4 + t3 - t2)
        MPDO = MPDO.permute(permute_shape)
    else:
        assert MPDO.ndim == 1 + 2*nqubits
        batch_size = MPDO.shape[0]
        #左乘
        permute_shape = list( range(2*nqubits+1) )
        permute_shape[wires + 1] = 2*nqubits
        permute_shape[2*nqubits] = wires + 1
        MPDO = MPDO.permute(permute_shape).unsqueeze(-1)
        # t2 = time.time()
        MPDO = ( matrix @ MPDO ).squeeze(-1)
        # t3 = time.time()
        ...
        #右乘
        permute_shape = list( range(2*nqubits+1) )
        permute_shape[wires+nqubits+1] = 2*nqubits
        permute_shape[2*nqubits] = wires+nqubits+1
        MPDO = MPDO.permute(permute_shape).unsqueeze(-2)
        
        
        # t4 = time.time()
        MPDO = MPDO.reshape([batch_size]+[2**(2*nqubits-1),2])
        MPDO = ( MPDO @ matrix.permute(1,0).conj() ).squeeze(-2)
        MPDO = MPDO.reshape([batch_size]+[2]*2*nqubits)
        # t5 = time.time()
        # print("乘法耗时2：",t5 - t4)
...
def _TwoQbitGate_TN_contract_Rho(nqubits, wires, matrix,\
                                     MPDO:torch.Tensor, batch_mod:bool=False)->torch.Tensor:
    '''
    2022-02-21 ssy
    '''
    if nqubits == -1 or wires == -1:
        raise ValueError("_TwoQbitGate_TN_contract_Rho input error! cannot TN_contract")
    cqbit = wires[0]
    tqbit = wires[1]
    assert cqbit != tqbit
    # assert matrix.shape[0] == 4 and matrix.shape[1] == 4
    if batch_mod==False:
        assert len(MPDO.shape) == 2*nqubits
        #左乘
        permute_shape = list( range(2*nqubits) )
        ...
        MPDO = MPDO.view([2]*2*nqubits)
        permute_shape = list( range(2*nqubits) )
        permute_shape.pop()
        permute_shape.pop()
        if cqbit < tqbit:
            permute_shape.insert(cqbit,2*nqubits-2)
            permute_shape.insert(tqbit,2*nqubits-1)
        else:
            permute_shape.insert(tqbit,2*nqubits-1)
            permute_shape.insert(cqbit,2*nqubits-2)
        MPDO = MPDO.permute(permute_shape)
        # print("左乘之后：",MPDO.reshape([2**nqubits, 2**nqubits]))
        #右乘
        permute_shape = list( range(2*nqubits) )
        permute_shape.remove(cqbit+nqubits)
        permute_shape.remove(tqbit+nqubits)
        permute_shape = permute_shape + [cqbit+nqubits, tqbit+nqubits]
        ...
        MPDO = MPDO @ matrix.permute(1,0).conj()
        ...
        permute_shape.pop()
        if cqbit < tqbit:
            permute_shape.insert(cqbit+nqubits,2*nqubits-2)
            permute_shape.insert(tqbit+nqubits,2*nqubits-1)
        else:
            permute_shape.insert(tqbit+nqubits,2*nqubits-1)
            permute_shape.insert(cqbit+nqubits,2*nqubits-2)
        MPDO = MPDO.permute(permute_shape)
        # print("右乘之后：",MPDO.reshape([2**nqubits, 2**nqubits]))
        return MPDO

    else:
        assert len(MPDO.shape) == 1 + 2*nqubits
        batch_size = MPDO.shape[0]
        #左乘
        permute_shape = list( range(2*nqubits + 1) )
        permute_shape.remove(cqbit+1) #注意batch情况下要+1
        ...
        MPDO = MPDO.view([batch_size] + [2]*2*nqubits)
        permute_shape = list( range(2*nqubits + 1) )
        permute_shape.pop()
        permute_shape.pop()
        if cqbit < tqbit:
            permute_shape.insert(cqbit+1,2*nqubits-1)
            permute_shape.insert(tqbit+1,2*nqubits)
        else:
            permute_shape.insert(tqbit+1,2*nqubits)
            permute_shape.insert(cqbit+1,2*nqubits-1)
        MPDO = MPDO.permute(permute_shape)
        #右乘
        permute_shape = list( range(2*nqubits + 1) )
        permute_shape.remove(cqbit+nqubits+1)
        permute_shape.remove(tqbit+nqubits+1)
        permute_shape = permute_shape + [cqbit+nqubits+1, tqbit+nqubits+1]
        ...
        permute_shape.pop()
        if cqbit < tqbit:
            permute_shape.insert(cqbit+nqubits+1,2*nqubits-1) #注意batch右乘情况下要+nqubits+1
            permute_shape.insert(tqbit+nqubits+1,2*nqubits)
        else:
            permute_shape.insert(tqbit+nqubits+1,2*nqubits)
            permute_shape.insert(cqbit+nqubits+1,2*nqubits-1)
        MPDO = MPDO.permute(permute_shape)
        return MPDO
...
def _toffoli_TN_contract_Rho(nqubits, wires, matrix, MPDO:torch.Tensor, batch_mod:bool=False)->torch.Tensor:
    '''
    2022-02-21 ssy
    '''
    # print("_toffoli_TN_contract_Rho 相关函数太过复杂有待补充")
    if nqubits == -1 or wires == -1:
        raise ValueError("toffoli gate input error! cannot TN_contract_Rho")
    cqbit1 = wires[0]
    cqbit2 = wires[1]
    tqbit = wires[2]
    assert cqbit1 != tqbit and cqbit2 != tqbit and cqbit1 != cqbit2
    if batch_mod == False:
        ...
        permute_shape.pop()
        if cqbit1 < cqbit2 and cqbit2 < tqbit:
            permute_shape.insert(cqbit1, 2*nqubits-3)
            permute_shape.insert(cqbit2, 2*nqubits-2)
            permute_shape.insert(tqbit,  2*nqubits-1)
        elif cqbit1 < tqbit and tqbit < cqbit2:
            permute_shape.insert(cqbit1, 2*nqubits-3)
            permute_shape.insert(tqbit,  2*nqubits-1)
        ...
        #右乘
        permute_shape = list( range(2*nqubits) )
        permute_shape.remove(cqbit1 + nqubits)
        permute_shape.remove(cqbit2 + nqubits)
        permute_shape.remove(tqbit + nqubits)
        permute_shape = permute_shape + [cqbit1 + nqubits, cqbit2 + nqubits, tqbit + nqubits]
        ...
        permute_shape.pop()
        if cqbit1 < cqbit2 and cqbit2 < tqbit:
            permute_shape.insert(cqbit1 + nqubits, 2*nqubits-3)
            permute_shape.insert(cqbit2 + nqubits, 2*nqubits-2)
            permute_shape.insert(tqbit + nqubits,  2*nqubits-1)
        elif cqbit1 < tqbit and tqbit < cqbit2:
            permute_shape.insert(cqbit1 + nqubits, 2*nqubits-3)
            permute_shape.insert(tqbit + nqubits,  2*nqubits-1)
        ...
        permute_shape.pop()
        if cqbit1 < cqbit2 and cqbit2 < tqbit:
            permute_shape.insert(cqbit1 + 1, 2*nqubits-2)
            permute_shape.insert(cqbit2 + 1, 2*nqubits-1)
            permute_shape.insert(tqbit + 1,  2*nqubits)
        elif cqbit1 < tqbit and tqbit < cqbit2:
            permute_shape.insert(cqbit1 + 1, 2*nqubits-2)
            permute_shape.insert(tqbit + 1,  2*nqubits)
        ...
        #右乘
        permute_shape = list( range(2*nqubits + 1) )
        permute_shape.remove(cqbit1 + nqubits + 1)
        permute_shape.remove(cqbit2 + nqubits + 1)
        permute_shape.remove(tqbit + nqubits + 1)
        permute_shape = permute_shape + [cqbit1 + nqubits + 1, cqbit2 + nqubits + 1, tqbit + nqubits + 1]
        ...
        permute_shape.pop()
        if cqbit1 < cqbit2 and cqbit2 < tqbit:
            permute_shape.insert(cqbit1 + nqubits + 1, 2*nqubits-2)
            permute_shape.insert(cqbit2 + nqubits + 1, 2*nqubits-1)
            permute_shape.insert(tqbit + nqubits + 1,  2*nqubits)
        elif cqbit1 < tqbit and tqbit < cqbit2:
            permute_shape.insert(cqbit1 + nqubits + 1, 2*nqubits-2)
            permute_shape.insert(tqbit + nqubits + 1,  2*nqubits)
...
def _SingleGateLayer_TN_contract_Rho(nqubits, wires, lst1, MPDO:torch.Tensor, batch_mod:bool=False)->torch.Tensor:
    '''
    2022-02-21 ssy
    '''
    for qbit in wires:
        MPDO = _SingleGateOperation_TN_contract_Rho(nqubits, qbit, lst1[qbit], MPDO, batch_mod=batch_mod)
    return MPDO
...
def _MatrixProductDensityOperator(rho:torch.Tensor, N:int)->torch.Tensor:
    ...
    2022-02-21 ssy
    支持batched rho输入，第一个维度为batch size
    '''
    if len(rho.shape) == 3:
        #考虑batched rho输入
        if rho.shape[1] != 2**N or rho.shape[2] != 2**N:
            raise ValueError('MPDO: dimension of batched rho must be [batch_size, 2^N, 2^N]')
        batch_size = rho.shape[0]
        rho_tensor = rho.view( [batch_size]+[2]*2*N )
        assert len(rho_tensor.shape) == 1 + 2*N
        return rho_tensor
        ...
        raise ValueError('MPDO: rho must have 2 or 3(with batch) dimensions')
