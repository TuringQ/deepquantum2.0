...
from deepquantum.layers.qlayers import HLayer, XYZLayer, YZYLayer, XZXLayer, XZLayer, ZXLayer, \
    ring_of_cnot, ring_of_cnot2, BasicEntangleLayer, ZYXLayer, ZYZLayer
...
class Circuit(object):
    ...
    def state_init(self):
        ...
        '''
        state_init = torch.zeros(2 ** self.nqubits).to(self.device)
        state_init[0] = 1.0
    ...
    def U(self, left_to_right=True, cuda=False):
        ...
        U_overall = torch.eye(2 ** self.nqubits, 2 ** self.nqubits).to(self.device) + 0j
    ...
    def Hadamard(self, wires):
        if isinstance(wires, Iterable):
            self.add(HLayer(self.nqubits, wires, self.dev))
        else:
            self.add(Hadamard(self.nqubits, wires, self.dev))

    def PauliX(self, wires):
        self.add(PauliX(self.nqubits, wires, self.dev))

    def PauliY(self, wires):
        self.add(PauliY(self.nqubits, wires, self.dev))

    def PauliZ(self, wires):
        self.add(PauliZ(self.nqubits, wires, self.dev))

    def rx(self, theta, wires):
        self.add(rx(theta, self.nqubits, wires, self.dev))

    def ry(self, theta, wires):
        self.add(ry(theta, self.nqubits, wires, self.dev))

    def rz(self, theta, wires):
        self.add(rz(theta, self.nqubits, wires, self.dev))

    def u1(self, theta, wires):
        self.add(u1(theta, self.nqubits, wires, self.dev))

    def u3(self, theta_lst, wires):
        self.add(u3(theta_lst, self.nqubits, wires, self.dev))

    def rxx(self, theta, wires):
        self.add(rxx(theta, self.nqubits, wires, self.dev))

    def ryy(self, theta, wires):
        self.add(ryy(theta, self.nqubits, wires, self.dev))

    def rzz(self, theta, wires):
        self.add(rzz(theta, self.nqubits, wires, self.dev))

    def cnot(self, wires):
        self.add(cnot(self.nqubits, wires, self.dev))

    def cz(self, wires):
        self.add(cz(self.nqubits, wires, self.dev))

    def cphase(self, theta, wires):
        self.add(cphase(theta, self.nqubits, wires, self.dev))

    def cu3(self, theta_lst, wires):
        self.add(cu3(theta_lst, self.nqubits, wires, self.dev))

    def SWAP(self, wires):
        self.add(SWAP(self.nqubits, wires, self.dev))

    def toffoli(self, wires):
        self.add(toffoli(self.nqubits, wires, self.dev))

    def multi_control_cnot(self, wires):
        self.add(multi_control_cnot(self.nqubits, wires, self.dev))

    # ====================================================================
    def XYZLayer(self, wires, params_lst):
        self.add(XYZLayer(self.nqubits, wires, params_lst, self.dev))

    def ZYXLayer(self, wires, params_lst):
        self.add(ZYXLayer(self.nqubits, wires, params_lst, self.dev))

    def ZYZLayer(self, wires, params_lst):
        self.add(ZYZLayer(self.nqubits, wires, params_lst, self.dev))

    def YZYLayer(self, wires, params_lst):
        self.add(YZYLayer(self.nqubits, wires, params_lst, self.dev))

    def XZXLayer(self, wires, params_lst):
        self.add(XZXLayer(self.nqubits, wires, params_lst, self.dev))

    def XZLayer(self, wires, params_lst):
        self.add(XZLayer(self.nqubits, wires, params_lst, self.dev))

    def ZXLayer(self, wires, params_lst):
        self.add(ZXLayer(self.nqubits, wires, params_lst, self.dev))

    def ring_of_cnot(self, wires):
        self.add(ring_of_cnot(self.nqubits, wires, self.dev))

    def ring_of_cnot2(self, wires):
        self.add(ring_of_cnot2(self.nqubits, wires, self.dev))

    def BasicEntangleLayer(self, wires, params_lst, repeat=1):
        self.add(BasicEntangleLayer(self.nqubits, wires, params_lst, repeat, self.dev))
